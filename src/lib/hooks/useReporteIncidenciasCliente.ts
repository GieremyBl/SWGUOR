...
export function useReporteIncidenciasCliente(filters?: Filters) {
  const [data, setData] = useState<ReporteIncidenciasResponse | null>(null);
  ...
  const loadData = async () => {
    try {
      ...
      const response = await fetch(
        `/api/admin/reportes/incidencias-cliente?${params.toString()}`,
      );
